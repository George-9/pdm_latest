...
import {
  addPriestCommunityOptionToPicker,
  OutstationPicker,
} from "../tailored_ui/outstation_picker.js";
import {
  ExcelExportButton,
  PDFPrintButton,
} from "../tailored_ui/print_button.js";
import {
  Column,
  MondoText,
  TextEdit,
...
export function promptAddTitheView() {
  ...
  const unRecognizedMemberNameI = TextEdit({
    styles: [HIDDEN_STYLE],
    placeholder: "name",
  });
      ...
      if (titheCategoryPicker.value === TitheCategory.member) {
        if (!selectedMemberId) {
          return MessegePopup.showMessegePuppy([
            MondoText({ text: "select a member to continue" }),
          ]);
        }
        body = {
          tithe: {
  ...
  searchResultViewContainer = Column({
    styles: [{ height: "400px" }],
    classlist: ["f-h", "f-w", "scroll-y"],
    children: [],
  });
...
export function showTitheReportsView() {
  ...
  sccPicker.addEventListener("change", function (ev) {
    ...
    PDFPrintButton.printingHeading =
      ...
      " " +
      JSON.parse(sccPicker.value)["name"] +
      " SCC tithe records";

    selectedSCCTotalTithe = 0;
    const thiSCCMembers = getSCCMembers(
      sccPicker.value,
      outstationPicker.value
    );
  ...
  const mainColumn = Column({
    children: [
      Column({
        ...
        children: [outstationPicker, sccPicker],
      }),
      containerColumn,
    ],
  });
  ...
  viewTotalsForEachSCCButton.onclick = function (ev) {
    ...
    PDFPrintButton.printingHeading =
      LocalStorageContract.completeParishName() + " outstations tithe records'";
    ...
    for (let l = 0; l < ParishDataHandle.parishOutstations.length; l++) {
      ...
      for (let j = 0; j < ParishDataHandle.parishTitheRecords.length; j++) {
        ...
        for (let k = 0; k < ParishDataHandle.parishMembers.length; k++) {
          ...
          if (
            member &&
            member["outstation_id"] === outstation["_id"] &&
            member["_id"] === titheRecord["member_id"]
          ) {
            outstationTotalTithe += amount;
          }
        }
    ...
    const excelExportButton = ExcelExportButton(
      tableId,
      ParishDataHandle.parishTitheRecords
    );
  ...
  viewUrecognizedMembersTitheButton.onclick = function (ev) {
    const unkownMmebersTitheRecords =
      ParishDataHandle.parishTitheRecords.filter(function (titheRecord) {

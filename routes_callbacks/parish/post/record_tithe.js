...
export async function addTitheRecord(req, resp) {
  ...
  if (await parishExists(parish_code, parish_password)) {
    ...
    const amount = tithe["amount"];
    delete tithe["amount"];

    let saveResult = await MongoDBContract.dbInstance(parish_code)
      .collection(DBDetails.titheCollection)
      .updateOne(
        tithe,
        { $set: { ...tithe, amount: amount } },
        { upsert: true }
      );

    return resp.json({
      response: saveResult ? "success" : "something went wrong",
    });
  } else {
...
export async function addTitheRecordByOutstation(req, resp) {
  ...
  if (await parishExists(parish_code, parish_password)) {
    let saveResult = await MongoDBContract.insertIntoCollection(
      tithe,
      parish_code,
